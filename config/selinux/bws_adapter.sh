...
if [ $# -eq 1 ]; then
	if [ "$1" = "--update" ] ; then
		time=`ls -l --time-style="+%x %X" bws_adapter.te | awk '{ printf "%s %s", $6, $7 }'`
		rules=`ausearch --start $time -m avc --raw -se bws_adapter`
		if [ x"$rules" != "x" ] ; then
			...
			if [ "$ANS" = "y" -o "$ANS" = "Y" ] ; then
				...
				echo -e "$rules" | audit2allow -R >> bws_adapter.te
...
set -x
make -f /usr/share/selinux/devel/Makefile bws_adapter.pp || exit
